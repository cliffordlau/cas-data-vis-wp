...
Remove the following entirely. Alternatively, use gray font or line color to tone-down the cognitive load of the following:

* Borders - generally not needed and obscure key data ink
...
**After** we have applied basic data visualization techniques, we can clearly tell what the message from the title and color choices. The axis amounts and other state's information is visible but given less emphasis with gray or muted color.

```{r }
knitr::include_graphics('figures/market_share_right.png')
```

* No gridlines, border, or axis lines. We don't miss them as we can now focus on the data
